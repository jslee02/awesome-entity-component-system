...
def apply_fixes(fixes: dict[str, str], root: Path) -> dict[str, list[tuple[str, str]]]:
...
def generate_pr_summary(redirect_fixes, wayback_fixes, changes, unfixable) -> str:
    lines = [
        ...
        f"- **{sum(len(v) for v in changes.values())}** total replacement(s) across **{len(changes)}** file(s)\n",
    ]
    if redirect_fixes:
        lines += [
            "### Redirected URLs\n",
...
def generate_unfixable_report(unfixable) -> str:
    lines = [
        "## Broken Links Requiring Manual Fix\n",
        "The following links could not be automatically fixed (no redirect detected and no Wayback Machine archive available).\n",
    ]
    for source, entries in sorted(unfixable.items()):
